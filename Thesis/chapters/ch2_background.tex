...
\item \textbf{Output-hash introspection.} The script computes a hash over all outputs
  ...
  uses this approach. The script sees only the aggregate hash, not individual output
  fields.
...
\item \textbf{Per-output introspection.} The script examines individual outputs by
  ...
  expected Taproot structure. Simplicity on Elements~(Appendix~\ref{app:simplicity})
  also belongs here, binding output 0 via
  \texttt{jet::output\_script\_hash(0)} and \texttt{jet::output\_amount(0)}.
...
(\texttt{jet::output\_script\_hash()}, \texttt{jet::output\_amount()},
\texttt{jet::sig\_all\_hash()}), timelocks (\texttt{jet::check\_lock\_distance()}), and
cryptographic primitives (\texttt{jet::bip\_0340\_verify()}~\cite{BIP340}) provide the
building blocks for covenant enforcement.

Our Simplicity vault enforces covenants by binding output 0 via two jets:
\texttt{jet::output\_script\_hash(0)} (scriptPubKey) and
\texttt{jet::output\_amount(0)} (asset and amount). Each Simplicity program
(trigger, withdraw, recover) compares the jet results against values baked in at
compile time; any divergence fails the program at the consensus level. Per-output
binding is the basis for bounded partial withdrawal without reshaping the covenant;
a full capability summary is in Appendix~\ref{app:simplicity}.
...
\begin{tikzpicture}[
  ...
  \node[paramnodestyle] (paramnode) at (0, 0.0) {$H$ = expected scriptPubKey hash (output 0)};

  % ===== Spend-time column =====
  \node[tx] (spendtx) at (5.2, 2.6) {Spending TX\\ \footnotesize outputs: [out1, out2]};
  \node[jetbig] (jeteval) at (5.2, 0.5) {\ttfamily jet::output\_script\_hash(0)};
...
(a DAG of combinators) embeds an expected scriptPubKey hash~$H$ for output 0 as a parameter.
At spend time, \texttt{jet::output\_script\_hash(0)} computes the scriptPubKey
hash of output 0. If the computed hash matches~$H$, the program accepts;
otherwise the program fails at the consensus level. Both trigger and recovery
leaves bind output 0 with this jet (and with \texttt{jet::output\_amount(0)}
for the asset and amount); a full capability summary is in
