...
Chapter~\ref{ch:verification}. The corpus spans nine \texttt{.als}
files with 42 assertions; 41 are invoked by \texttt{check} commands
(the assertion \texttt{ccvEventualWithdrawal} is defined but not
exercised by a \texttt{check} under the current scope). Of the 41
checks, 40 pass under the configured scopes and one
(\texttt{catcsfsNoExtraction\_ColdKeyOnly}) produces the expected
counterexample documenting CAT+CSFS's unconstrained cold-key
recovery.
...
Each of the five concrete models extends \texttt{vault\_base} with
covenant-specific transition guards. \texttt{ctv\_vault} models the
template-hash commitment. \texttt{ccv\_vault} models the CCV mode
enumeration and the OP\_SUCCESS behavior for undefined modes.
\texttt{opvault\_vault} models the three-key separation and
authorized recovery. \texttt{cat\_csfs\_vault} models the
dual-verification binding and unconstrained recovery leaf.
\texttt{simplicity\_vault} models the per-output binding jets
(\texttt{jet::output\_script\_hash(0)}, \texttt{jet::output\_amount(0)})
on both trigger and recovery paths.
...
\paragraph{CCV.}
Eight assertions are defined; seven are under \texttt{check} commands and all seven pass, including the mode confusion containment check (undefined
