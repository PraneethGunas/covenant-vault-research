...
show_help() {
    cat <<'EOF'
════════════════════════════════════════════════════════════
  Covenant Vault Comparison — Docker
════════════════════════════════════════════════════════════

Commands:
  run <experiment> --covenant <ctv|ccv|opvault|cat_csfs|simplicity|all>
  run --tag <tag> --covenant <ctv|ccv|opvault|cat_csfs|simplicity|all>
...
Experiments (15):
  ...
  fee_pinning                   Fee mechanism comparison and pinning resistance
  fee_sensitivity               Fee-dependent security inversion analysis
  recovery_griefing             Forced-recovery griefing cost analysis
  watchtower_exhaustion         Splitting attack against watchtower recovery budget
  ...
  ccv_mode_bypass               BIP-443 OP_SUCCESS behavior verification (CCV only)
  opvault_trigger_key_theft     Trigger key theft and recovery race economics (OP_VAULT only)
  opvault_recovery_auth         Authorized recovery cost and anti-griefing tradeoff (OP_VAULT only)
  cat_csfs_hot_key_theft        Dual-verification output binding verification (CAT+CSFS only)
  cat_csfs_witness_manipulation Sighash-preimage tamper resistance (CAT+CSFS only)
  cat_csfs_destination_lock     Destination rigidity cost and recovery escape (CAT+CSFS only)
  cat_csfs_cold_key_recovery    Cold key recovery analysis and alternative design (CAT+CSFS only)

Tags:
  core                          Primary experiments (lifecycle, attacks, comparisons)
  security                      Attack surface experiments
  verification                  Empirical confirmation of specified behavior
  ...
  cat_csfs_only                 CAT+CSFS-specific experiments
...
Covenants:
  ...
  simplicity                    Simplicity — Elements node (simplex regtest)
  all                           Run on all covenants (auto-switches nodes)
...
Examples:
  # Run all core experiments across all 5 covenants
  ...
  # Run lifecycle on Simplicity (Elements regtest)
  docker run -v ./results:/data/results vault-comparison run lifecycle_costs --covenant simplicity
...
Node binaries (inside container):
  ...
  /opt/simplex-bin/             elementsd + electrs + simplex (Simplicity)
  simple-simplicity-vault/      vault-cli (pre-built, target/release/)
EOF
    exit 0
}
