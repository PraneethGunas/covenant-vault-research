...
6. **Two original vault implementations.** The CAT+CSFS vault and the Simplicity vault are original implementations built for this research, not forks of existing code. The CAT+CSFS vault is the first working vault that uses dual Schnorr verification (OP\_CHECKSIGFROMSTACK + OP\_CHECKSIG) for covenant enforcement on Bitcoin Inquisition regtest. The Simplicity vault is built with the Simplicity language on Elements regtest via the Simplex SDK, using `jet::outputs_hash()` for covenant enforcement — the first Simplicity-based vault with measured lifecycle costs. Both implementations are open-source.
...
### 3.8 Variant Framework

The reference vault per opcode sits at one corner of a 4-axis design lattice. Each adapter declares additional **variants** it can construct by flipping one or more axes. Variants share the adapter's RPC plumbing and lifecycle interface but emit different scripts.

#### Axes (canonical names)

| Axis | Variable | Values |
|---|---|---|
| Fee model         | `f` | `anchor`, `wallet`, `inval`, `acp`, `dynamic` |
| Amount policy     | `a` | `atomic`, `partial` |
| Recovery gating   | `g` | `key`, `keyless` |
| Recovery binding  | `b` | `bound`, `unbound` |

#### Two anchors (non-dominated landmarks)

The four-axis lattice has two non-dominated landmark corners on the recovery-gating axis. Both have $f \neq f_{\text{anchor}}$, $a = \text{atomic}$, $b = \text{bound}$:

- **MES anchor** $D^\star_{\text{MES}} = (\neq\text{anchor}, \text{atomic}, \text{key}, \text{bound})$ — the foundational Möser-Eyal-Sirer (2016) design. Eliminates classes Z1, Z2, Z3, Z5 simultaneously.
- **Post-MES anchor** $D^\star_{\text{keyless}} = (\neq\text{anchor}, \text{atomic}, \text{keyless}, \text{bound})$ — the Ingala (CCV) / `simple-ctv-vault` design school. Eliminates Z1, Z3, Z5; admits Z2 by design (in exchange for keyless recoverability under cold-key loss).

Proposition 1 (griefing-safety dichotomy) is the formal statement of why neither anchor dominates the other. The operator's threat-priority weighting selects which anchor.

#### Axis → attack-class mapping

| Axis value | Attack class enabled | Threat model |
|---|---|---|
| `f_anchor`        | Z1 (fee-channel pinning) | TM1 |
| `a_partial`       | Z3 (per-UTXO recovery scaling) | TM4 |
| `g_keyless`       | Z2 (permissionless recovery griefing) | TM2 |
| `b_unbound`       | Z5 (cold-key destination redirection) | TM11 |
| `multi-mode + OP_SUCCESS` | Z6 (mode/parameter bypass) | TM8 |

Each class is the deterministic consequence of the corresponding axis value. Variants flipping one axis close exactly the corresponding class; multiple-axis variants close the union.

#### Registered variants per opcode

Variant ids are declared in each adapter's `VARIANTS` class attribute. Adapter `axes()` returns the active variant's `(f, a, g, b)` tuple; capability flags (`supports_revault()`, `supports_keyless_recovery()`) derive from the tuple, so experiments dispatch automatically.

| Opcode | Variant | $(f,a,g,b)$ | Anchor reached |
|---|---|---|---|
| CTV       | reference         | (anchor, atomic, keyless, bound) | — (locked at $f_{\text{anchor}}$) |
| CTV       | keygated          | (anchor, atomic, key, bound) | — |
| CCV       | reference         | (inval, partial, keyless, bound) | — |
| CCV       | atomic            | (inval, atomic, keyless, bound) | post-MES |
| CCV       | keygated          | (inval, partial, key, bound) | — |
| CCV       | keygated-atomic   | (inval, atomic, key, bound) | **MES** |
| OP\_VAULT | reference         | (wallet, partial, key, bound) | — |
| OP\_VAULT | keyless           | (wallet, partial, keyless, bound) | — |
| OP\_VAULT | atomic            | (wallet, atomic, key, bound) | **MES** |
| OP\_VAULT | keyless-atomic    | (wallet, atomic, keyless, bound) | post-MES |
| CAT+CSFS  | reference         | (acp, atomic, key, unbound) | — |
| CAT+CSFS  | bound             | (acp, atomic, key, bound) | MES (modulo $f_{\text{acp}}$) |

CTV cannot reach either anchor: template rigidity forces `f_anchor` in any vault construction. The only constructible CTV variant is the keygated cold-leg flip.

A "keyless-bound" CAT+CSFS variant is **not** constructible without CTV-equivalent template enforcement: pure CSFS without `OP_CHECKSIG` does not bind the actual spending tx to the recovery template (the script verifies a hash on witness-provided bytes, detached from the real sighash).

#### Variant-aware experiments

Experiments dispatch on `adapter.capabilities()` rather than `adapter.name`. The harness threads the active variant via `adapter.setup(rpc, variant=...)`. Two CLI knobs:
...
- **OP_VAULT keyless** (BIP-345 unauthorised mode): the recovery tx must have exactly two outputs (recovery + ephemeral anchor with `nValue=0` and `scriptPubKey=OP_2`); the parent has zero fee, paid via a CPFP child submitted together via `submitpackage`.
- **CAT+CSFS bound**: the cold leaf mirrors the trigger leaf's CSFS+CAT introspection pattern, with the embedded `sha_single_output` committing to the recovery destination. The witness carries two signatures (CSFS + CHECKSIG) and a sighash-preimage prefix/suffix split.
- **CCV keygated**: a new clause `recover_keygated` is appended alongside the keyless `recover` clause; the witness order is `[<out_i>, <sig>]` so `OP_CHECKSIGVERIFY` pops the sig first.
- **CTV keygated**: the cold-leg in `unvault_redeemScript` becomes `<cold_pubkey> OP_CHECKSIGVERIFY <recover_template_hash> OP_CTV`; the witness for the cold path adds the cold signature beneath the false `OP_IF` selector.
...
- TM4 (per-UTXO recovery-cost scaling): **N/A** — no revault capability, single unvault of full amount (like CTV/CAT+CSFS).
