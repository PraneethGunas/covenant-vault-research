...
\texttt{cross\_covenant}) are opened by the concrete models. The
corpus spans nine \texttt{.als} files with 42 assertions, of which 41
are invoked by \texttt{check} commands (one assertion,
\texttt{ccvEventualWithdrawal}, is defined but not checked). Of the
41 checks, 40 pass under the configured scopes and one
(\texttt{catcsfsNoExtraction\_ColdKeyOnly}) produces the expected
counterexample documenting \catcsfs{}'s unconstrained cold-key
recovery.
...
\texttt{simplicity\_vault.als} models the per-output binding jets
...
\paragraph{\opccv{}.} Eight assertions are defined; seven are under
\texttt{check} commands and all seven pass, including a
mode-confusion containment check (undefined modes trigger
\texttt{OP\_SUCCESS} but only within the affected leaf; the
vault's other leaves remain secure). The eighth assertion
(\texttt{ccvEventualWithdrawal}) is defined for reference but not
exercised by a \texttt{check} under the current scope. The griefing
scenario demonstrates that any party may invoke recovery without
keys (TM2). The splitting scenario exhibits the revault chain that
