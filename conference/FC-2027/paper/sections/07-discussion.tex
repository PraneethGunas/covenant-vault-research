...
Alloy's finite-scope analysis (42 assertions across nine files:
five concrete covenant models atop shared \texttt{btc\_base},
\texttt{vault\_base}, \texttt{threat\_model}, and
\texttt{cross\_covenant} modules; 41 under \texttt{check} commands,
40 passing and one expected-to-fail counterexample on
\texttt{catcsfsNoExtraction\_ColdKeyOnly}) provides refutation for
broad claims but does not constitute a full proof. Mechanised
proofs (Coq, Lean) of Proposition~\ref{thm:griefing_safety} and of
the feature-vulnerability implications D1--D4
(Section~\ref{sec:impossibility}) remain open.
